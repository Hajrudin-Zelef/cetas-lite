...
func New(cfg *config.Config, st *store.Store, authMgr *auth.Manager, engine *chat.Engine, termMgr *terminal.Manager, registry *provider.Registry, httpClient *http.Client, version string) *Server {
	...
	mux.HandleFunc("GET /api/settings/credits", s.requireAuth(s.handleCreditsGet))
	mux.HandleFunc("POST /api/settings/credits", s.requireAuth(s.handleCreditsPost))
	mux.HandleFunc("POST /api/settings/credits/refresh", s.requireAuth(s.handleCreditsRefresh))
	...
	mux.HandleFunc("GET /api/deepthink", s.requireAuth(s.handleDeepThinkGet))
	mux.HandleFunc("PUT /api/deepthink", s.requireAuth(s.handleDeepThinkPut))
	mux.HandleFunc("POST /api/deepthink/translate", s.requireAuth(s.handleDeepThinkTranslate))
	mux.HandleFunc("GET /api/local/models", s.requireAuth(s.handleLocalModels))
	mux.HandleFunc("GET /api/local/engines", s.requireAuth(s.handleLocalEnginesList))
	mux.HandleFunc("PUT /api/local/engines/{id}", s.requireAuth(s.handleLocalEnginePut))
	mux.HandleFunc("DELETE /api/local/engines/{id}", s.requireAuth(s.handleLocalEngineDelete))
	mux.HandleFunc("POST /api/local/engines/{id}/test", s.requireAuth(s.handleLocalEngineTest))
	...
	mux.HandleFunc("POST /api/chat/prefetch", s.requireAuth(s.handleChatPrefetch))
	...
	mux.HandleFunc("GET /api/sessions/current", s.requireAuth(s.handleSessionCurrent))
	mux.HandleFunc("POST /api/sessions/{id}/open", s.requireAuth(s.handleSessionOpen))
	mux.HandleFunc("DELETE /api/sessions/{id}", s.requireAuth(s.handleSessionDelete))
	mux.HandleFunc("GET /api/sessions/{id}/export", s.requireAuth(s.handleSessionExport))
