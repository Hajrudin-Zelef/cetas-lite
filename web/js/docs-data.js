      ...
      articles: [
        ...
        },
        {
          ...
          sections: [
            ...
            { h: "Quel modèle choisir ?", html: `<p>Pour discuter, un modèle léger suffit. Pour du code ou des missions complexes, préférez une version <em>standard/elite</em>. Vous pouvez changer d'alias à chaque message sans casser le fil de la discussion. Le <strong>menu +</strong> liste les alias (Nano, N4, SamGen) en <strong>cascade</strong> : survolez un alias, puis une version. À chaque niveau, <strong>Auto</strong> = le <em>fallback</em> (tirage aléatoire sur tout l'ensemble) ; en dessous, <strong>Models</strong> permet de choisir <em>manuellement</em> un modèle (Nano : <em>Free de OpenRouter</em>, <em>Zen Free</em>). Dans <strong>Configuration → Sélecteur de modèles</strong>, réglez chaque version sur <strong>1 modèle</strong> (fixe) ou <strong>Fallback</strong> (tirage aléatoire à chaque requête) — le bouton Fallback re-coche désormais tout l'ensemble.</p>` },
          ],
        },
        {
          id: "premier-message",
    ...
    },
    {
      ...
      articles: [
        ...
        },
        {
          ...
          sections: [
            { h: "Afficher le raisonnement", html: `<p>Le bouton <strong>Thinking</strong> demande au modèle d'exposer son raisonnement avant de répondre. Dans le chat, il est désactivé par défaut ; dans la vue Agents, il est toujours actif.</p><p>Le raisonnement s'affiche dans un panneau latéral dédié, en temps réel pendant la génération.</p>` },
            { h: "Niveau d'effort", html: `<p>Le sélecteur d'<strong>effort</strong> (défaut, faible, moyen, max) règle la profondeur du raisonnement : un effort élevé donne des réponses plus fouillées, au prix d'un temps de génération plus long.</p>` },
          ],
        },
        {
          id: "tokens-couts",
