---
id: ai-industry-kb-2026-wave6/10-mistral-ai/sources-and-urls
title: "Sources and URLs"
domain: mistral-ai
role: deep-dive
task: actor-profile
actors: ["EU", "Hugging Face", "Microsoft", "Mistral", "Nvidia", "OpenRouter", "Samsung"]
dates: ["2025-07", "2025-12-02", "2025-12-09", "2026-03-28", "2026-04-28", "2026-05-22", "2026-08-31", "2026-09-21"]
keywords: ["agent", "agentic", "agents", "apache", "arr", "benchmark", "benchmarks", "claude", "compute", "consumer", "deepseek", "distillation"]
source: docs/RAG/ai-industry-knowledge-base-2026-wave6.md
source_anchor: ""
source_lines: [5048, 5148]
section: "§10. Mistral AI"
sha256: 7c96e2adce89e629253f62b9d7aafd9850950fc1223490d4fc8195c73f27b6af
---

# Sources and URLs

## Sources and URLs
- https://mistral.ai/news/devstral-2507/
- https://techcrunch.com/2025/12/09/mistral-ai-surfs-vibe-coding-tailwinds-with-new-coding-models/
- https://aidailypost.com/news/mistral-ai-launches-devstral-2-123b-devstral-small-2-24b-vibe-cli
- https://ascii.co.uk/news/article/news-20251209-da22d3ac/mistral-launches-devstral-2-coding-model-and-vibe-cli
- https://aitech365.com/business-technology/it-and-devops/mistral-launches-devstral-2-and-vibe-cli-coding-tools/
- https://aigazine.net/industry/mistral-unveils-ministral-3-14b-8b-and-3b-models-built-with-cascade-distillation--v
- https://fourweekmba.com/ai-nvidia-meta-open-weights-coalition-distillation-policy/


### New sources — expansion

- S1 — Secondary. OpenRouter model page for Mistral Medium 3.1 (pricing, context, cutoff, Devstral 2 2512 listing): https://openrouter.ai/mistralai/mistral-medium-3.1
- S2 — Vendor. Mistral docs page for Mistral Medium 3.1 (deprecation 2026-05-22, features, Shieldstral entry): https://docs.mistral.ai/models/mistral-medium-3-1-25-08
- S3 — Secondary. Magica model specs for Mistral Medium 3.1: https://magica.com/blog/model/mistral-medium-3-1
- S4 — Vendor. Mistral platform changelog 2026-04-28 (Medium 3.5 release date): https://github.com/mistralai/platform-docs-public/blob/HEAD/changelog/en/2026-04-28.mdx
- S5 — Secondary. Research notes on Mistral Medium 3.5 (128B, Modified MIT, 77.6 SWE-Bench): https://github.com/t0msilver/working-set/blob/HEAD/research/model_mistral_medium35.md
- S6 — Secondary. Issue tracker referencing Medium 3.5: https://github.com/a5c-ai/babysitter/issues/360
- S7 — Secondary. MungoMash Mistral versions page (Large 3 specs, distribution): https://mungomash.com/ai/mistral/versions/
- S8 — Secondary. LocalAIMaster on Mistral Large (pricing, context): https://localaimaster.com/models/mistral-large-123b
- S9 — Secondary. AwesomeAgents on Mistral Small 4 (MoE config, reasoning_effort, vendor claims): https://awesomeagents.ai/news/mistral-small-4-moe-apache-configurable-reasoning/
- S10 — Secondary. AI Tool Watch model-access notes for Mistral Small 4: https://github.com/snapsynapse/ai-tool-watch/blob/HEAD/data/model-access/mistral-small-4.md
- S11 — Secondary. AIForAutomation on Small 4 launch: https://aiforautomation.io/news/2026-03-28-mistral-small-4-119b-free-beats-gpt4o-mini
- S12 — Secondary. Local-run guide for Small 4 (hardware guidance): https://medium.com/@Sebastien67/running-mistral-small-4-119b-nvfp4-locally-on-a-dgx-spark-81cc2fdc4f6f
- S13 — Secondary. FourWeekMBA on Series D, compute, acquisitions, Robostral, Ministral 3: https://fourweekmba.com/ai-mistral-series-d-samsung-nvidia-asml-sovereign-ai/
- S14 — Secondary. Slator on the €3B Series D: https://slator.com/mistral-ai-raises-3-billion-series-d/
- S15 — Secondary. AIFrontPage on the record European equity round: https://aifront-page.com/mistral-ai-3-billion-funding-largest-european-tech-equity-round/
- S16 — Secondary. Pulse2 on Series D investors and valuation: https://pulse2.com/mistral-raises-e3-billion-series-d-at-more-than-e21-billion-valuation-led-by-samsung/
- S17 — Secondary. The Decoder on the Le Chat → Vibe rebrand: https://the-decoder.com/mistral-rebrands-lechat-as-vibe-betting-its-chatbots-future-is-as-a-full-blown-work-agent/
- S18 — Secondary. Requesty provider listing for Codestral-latest (updated 2026-09-21): https://www.requesty.ai/models/mistral/codestral-latest
- S19 — Secondary. Teradata Loom Mistral LLM reference (catalog pricing): https://github.com/teradata-labs/loom/blob/HEAD/docs/reference/llm-mistral.md
- S20 — Secondary. TMTPost on Mistral enterprise positioning: https://en.tmtpost.com/news/7448938
- S21 — Secondary. Adam Holter on Devstral Small 2507 SWE-Bench results: https://adam.holter.com/devstral-small-2507-mistral-ais-agentic-coding-llm-just-destroyed-the-swe-bench/
- S22 — Secondary. SecurityBoulevard on the Devstral launch (All Hands AI, Apache 2.0): https://securityboulevard.com/2025/05/mistral-launches-devstral-open-source-llm-for-coding-agents/
- S23 — Secondary. Gadgets360 on Devstral release (128K context, API pricing): https://www.gadgets360.com/ai/news/mistral-devstral-agentic-coding-ai-model-open-source-released-features-outperforms-openai-gpt-4-1-mini-8478674?ref=upmarket.co
- S24 — Secondary. APIDog on Devstral Small/Medium 2507 (July 10 release, benchmarks): https://apidog.com/blog/devstral-small-medium-2507/
- S25 — Secondary. TechJuice on Devstral unveiling: https://www.techjuice.pk/mistral-ai-unveils-devstral-an-open-source-llm-for-developers/
- S26 — Secondary. AIREVOLUTION on the Magistral launch (benchmarks, Flash Answers): http://airevolution.poltextlab.com/mistral-ai-unveils-its-first-reasoning-model-10x-faster-than-competitors/
- S27 — Secondary. TheAITrack on Magistral (Macron backing, multilingual reasoning): https://theaitrack.com/mistral-magistral-europe-reasoning-ai-launch/
- S28 — Secondary. AI-release tracker on Magistral (official announcement link): https://github.com/sbley/claude-code-agent-test/issues/497
- S29 — Secondary. VARIndia on Magistral dual-model strategy: https://www.varindia.com/news/mistral-takes-on-openai-deepseek-with-new-magistral-models
- S30 — Secondary. TechCrunch on the Magistral pair (critical benchmark assessment): https://techcrunch.com/2025/06/10/mistral-releases-a-pair-of-ai-reasoning-models/
- S31 — Community. Awesome-Mistral catalog (retirements, repos, licenses, OCR 4.1): https://github.com/samouraiworld/awesome-mistral/blob/HEAD/README.md
- S32 — Secondary. VentureBeat on Mistral OCR 3 launch: https://venturebeat.com/infrastructure/mistral-launches-ocr-3-to-digitize-enterprise-documents-touts-74-win-rate
- S33 — Secondary. VentureBeat on Mistral OCR 4 (revenue targets, headcount): https://venturebeat.com/data/mistral-launches-ocr-4-turning-document-extraction-into-a-full-enterprise-ai-play
- S34 — Secondary. TechTimes on OCR 4 document-AI positioning: https://www.techtimes.com/articles/319207/20260627/document-ai-pipeline-errors-start-ocr-mistral-ocr-4-collapses-three-stages-one.htm
- S35 — Secondary. Scout-CI battlecard on Vibe, OCR 4 specs, pricing: https://github.com/uroshp/scout-ci/blob/HEAD/v2/battlecards/mistral__vs__openai__enterprise-and-sovereign-ai-for-developers-and-agents/current.md

- S36 — Secondary. SingularityByte on Medium 3.5 (verbatim Modified MIT carve-out): https://singularitybyte.com/models/mistral-medium-3-5.html
- S37 — Secondary. WinBuzzer on Medium 3.5 launch (benchmarks, license shift, NVIDIA NIM): https://winbuzzer.com/2026/05/02/mistral-medium-3-5-unified-flagship-chat-reasoning-code-xcxwbn/
- S38 — Secondary. LLM Radar EU review on Medium 3.5 (specs, sovereignty, pricing): https://www.llmradar.eu/models/mistral-medium-3-5
- S39 — Secondary. Mungomash license-tier map across Mistral/Mixtral releases: https://mungomash.com/ai/mistral/versions/
- S40 — Community. dev.to review of Medium 3.5 (license catch for big companies): https://dev.to/techsifted/mistral-medium-35-review-a-128b-open-weight-model-with-a-coding-agent-that-opens-prs-for-you-5a0i
- S41 — Community. scout-ci battlecard on Medium 3.5 pricing ($1.50/$7.50) and $20M revenue threshold: https://github.com/uroshp/scout-ci/blob/HEAD/v2/battlecards/mistral__vs__openai__enterprise-and-sovereign-ai-for-developers-and-agents/current.md
- S42 — Community. sota-reference licensing table (Apache/MIT/Modified MIT landscape): https://github.com/boreilly-dc/sota-reference/blob/HEAD/models/local-llms-consumer-hardware.md

- S43 — Secondary. VentureBeat on Devstral 2 launch (123B Modified MIT, verbatim $20M clause, Small 2 Apache 2.0): https://venturebeat.com/ai/mistral-launches-powerful-devstral-2-coding-model-including-open-source
- S44 — Secondary. TechCrunch on Devstral 2 + Vibe CLI (2025-12-09, pricing, ≥4 H100s): https://techcrunch.com/2025/12/09/mistral-ai-surfs-vibe-coding-tailwinds-with-new-coding-models/
- S45 — Secondary. Geeky Gadgets on Devstral 2 overview (specs, Vibe limits): https://www.geeky-gadgets.com/mistral-devstral-2-overview/
- S46 — Secondary. SiliconANGLE on Devstral 2 pricing and licensing: https://www.siliconangle.com/2025/12/09/mistral-ais-devstral-2-open-weights-vibe-coding-model-built-rival-best-proprietary-systems/
- S47 — Community/secondary. dev.to developer guide on Large 3 (specs, benchmarks, pricing, 3000 H200s): https://dev.to/jangwook_kim_e31e7291ad98/mistral-large-3-the-675b-open-weight-moe-model-developer-guide-250a
- S48 — Secondary. TechCrunch on Mistral 3 family launch (2025-12-02, Ministral 3 nine models): https://techcrunch.com/2025/12/02/mistral-closes-in-on-big-ai-rivals-with-mistral-3-open-weight-frontier-and-small-models/
- S49 — Community. agentone free-tier research on Large 3 (Elo, self-hosting, free tiers): https://github.com/dirk-makerhafen/agentone/blob/HEAD/docs/free-tier-research/models/mistral-large-3.md

- S50 — Secondary. The AI Track on Magistral launch (London Tech Week, Small/Medium, Macron backing): https://theaitrack.com/mistral-magistral-europe-reasoning-ai-launch/
- S51 — Secondary. AIRevolution on Magistral announcement (benchmarks, multilingual, 10× throughput): http://airevolution.poltextlab.com/mistral-ai-unveils-its-first-reasoning-model-10x-faster-than-competitors/
- S52 — Secondary. VentureBeat on Magistral 1.2 updates (vision, benchmarks, API IDs): https://venturebeat.com/ai/mistrals-updated-magistral-small-1-2-reasoning-model-can-analyze-images-and
- S53 — Secondary. dev.ua on Magistral Small/Medium 1.2 (quantized on RTX 4090/MacBook): https://dev.ua/en/news/mistral-1758260437
- S54 — Community. AllAboutAI on Magistral multimodal benchmarks (MMMU, MathVista): https://www.allaboutai.com/ai-agents/magistral/
- S55 — Secondary. FourWeekMBA on Series D (€3B, >€21B, Samsung led, syndicate): https://fourweekmba.com/ai-mistral-series-d-samsung-nvidia-asml-sovereign-ai/
- S56 — Secondary. HPCwire AIwire on Series D (frontier research, infrastructure, 125+ customers): https://www.hpcwire.com/aiwire/2026/09/08/mistral-raises-e3b-to-expand-ai-research-and-infrastructure/
- S57 — Secondary. TechBriefly on Series D ($830M debt, 1GW by 2030, Luxembourg): https://techbriefly.com/2026/09/08/mistral-ai-raises-3-billion-samsung-led-funding-round/

- S37 — Community. Braintrust proxy issue (Medium 3/3.1 deprecated 2026-05-22, retired 2026-08-31, Medium 3.5 replacement): https://github.com/braintrustdata/braintrust-proxy/issues/922
- S38 — Community. Aitooltier sweep on Medium 3.1 retirement 2026-08-31: https://github.com/cmcmath84/aitooltier/commit/1ae148ac76919ecedfa633b1ef76471251a58e5a
- S39 — Secondary. The-Decoder on Medium 3.5 (SWE 77.6, T3-Telecom 91.4, replaces 3.1/Magistral/Devstral 2, Modified MIT): https://the-decoder.com/mistrals-new-flagship-medium-3-5-folds-chat-reasoning-and-code-into-one-model/
- S40 — Secondary. AI Dev Weekly on Medium 3.5 (Apr 29, dense 128B, 256K, Vibe remote agents): https://medium.com/@meulenjoske/ai-dev-weekly-8-mistral-medium-3-5-2bc7c0a7fcc7
- S41 — Vendor-adjacent. NVIDIA HF page for Mistral-Medium-3.5-128B-NVFP4 (replaces 3.1/Magistral, Modified MIT): https://huggingface.co/nvidia/Mistral-Medium-3.5-128B-NVFP4
- S42 — Secondary. Medium on Mistral Large 3 (Dec 2 2025, 675B/41B, 128 experts/layer, 256K): https://medium.com/@chiwai.kiriba/why-mistral-is-the-future-of-open-weight-intelligence-5fb967963643
- S43 — Vendor-adjacent. NVIDIA/Marktechpost on Large 3 inference (128 experts/layer, NVFP4, GB200 NVL72): https://www.marktechpost.com/2025/12/02/nvidia-and-mistral-ai-bring-10x-faster-inference-for-the-mistral-3-family-on-gb200-nvl72-gpu-systems/
- S44 — Secondary. Medium review of Mistral Large 3 (675B, ~40B active, ~3,000 H200 GPUs, AA Index placement): https://medium.com/@leucopsis/mistral-large-3-2512-review-7788c779a5e4
- S45 — Community. Red Hat deep dive on Mistral (Large 3 Apache 2.0, deployment, limitations): https://github.com/redhat-et/physical-ai-platform-intel/blob/HEAD/deliverables/intel/companies/mistral-ai-deep-dive.md
- S46 — Community. working-set research on Medium 3.5 (88 layers, GQA 12:1, dense 128B, config-derived params): https://github.com/t0msilver/working-set/blob/HEAD/research/model_mistral_medium35.md

- S45 — Community. Red Hat deep dive on Mistral AI (timeline, Emmi/Koyeb M&A, product specs, ARR): https://github.com/redhat-et/physical-ai-platform-intel/blob/HEAD/deliverables/intel/companies/mistral-ai-deep-dive.md
- S47 — Secondary. CRN on Mistral $830M debt for 13,800 GB300s (44 MW, Paris): https://www.crn.com/news/data-center/2026/mistral-ai-raises-830m-for-nvidia-data-center-with-13-800-gpus
- S48 — Secondary. Blockonomi on Mistral debt financing (seven banks, Bruyeres-le-Chatel, Q2 2026): https://blockonomi.com/mistral-ai-secures-830m-debt-financing-to-power-new-paris-data-center-with-nvidia-gpus/
- S49 — Secondary. Pulse on Mistral acquiring Emmi AI (Austrian LEM startup, physics simulation): https://www.pulse.bot/ai/deals/mistral-to-acquire-austrian-ai-startup-emmi-ai-78e5ec6b-19c3-4eb3-af90-c1df771f7d34/
- S50 — Secondary. AlternativeStack on Devstral Medium/Small 1.1 (61.6%/53.6% SWE-Bench): https://alternativestack.com/news/mistral-doubles-down-on-code-ai-devstral-medium-open-source-small-11-challenge-giants
- S51 — Secondary. MLQ on Devstral 2/Vibe CLI (53.6%/61.6% lineage, enterprise positioning): https://mlq.ai/news/mistral-ai-launches-devstral-2-coding-model-and-mistral-vibe-cli-for-vibe-coding-workflows/
- S52 — Secondary. Vocal.media on Devstral Medium (61.6% SWE-Bench, July 2025): https://vocal.media/futurism/how-devstral-s-ai-coding-agent-is-revolutionizing-development-in-2025-and-how-you-can-use-it
- S53 — Secondary. Onmine on Devstral launch (24B, Apache 2.0, $0.10/$0.30, OpenHands): https://onmine.io/mistral-ai-launches-devstral-powerful-new-open-source-swe-agent-model-that-runs-on-laptops/
- S54 — Secondary. Neurosignal on Devstral (finetuned from Small 3.1, 128K ctx, dogfooding): https://neurosignal.tech/mistral-ai-launches-devstral-powerful-new-open-source-swe-agent-model-that/

### Source-quality notes (methodology for this expansion)
- S31 (Awesome-Mistral) is a community-maintained catalog and the sole source for several retirement dates and Hugging Face repository names; retirement dates are described as published API dates from Mistral's official catalog.
- Medium 3.5's 77.6 SWE-Bench Verified figure appears only in S5 and is carried as UNVERIFIED.
- The Modified MIT verbatim carve-out is quoted from the LICENSE file via SingularityByte and corroborated by a battlecard citing Mistral's help article; the $20M/month figure appears in both. [SECONDARY, S36][SECONDARY, S41]
- Series D figures (€3B, >€21B) are corroborated across S13–S16 with consistent investor lists; the "largest European tech equity round" characterization is Mistral's own via S15.
- OCR 4's win-rate and OlmOCRBench figures are vendor claims via S35, with Mistral's own "directional rather than definitive" caveat preserved.

