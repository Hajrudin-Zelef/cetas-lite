---
id: collect-260926-rattrapage/rattrapage/github-socialwifi-routeros-api-python-api-to-routerboard-devices-produced-by-mikrotik
title: "This part here is important:"
domain: rattrapage
role: reference
task: reference
actors: []
...
source: docs/RAG/lot-rattrapage/ai-llm/github-socialwifi-routeros-api-python-api-to-routerboard-devices-produced-by-mikrotik.md
source_anchor: ""
source_lines: [1, 161]
sha256: bf9d75bc929ebb69e12c1028b273357e95715df6974800da3fe163eadef1436a
---
...
```
`list = api.get_resource('/command')``list_queues = api.get_resource('/queue/simple')``list_queues.get()``list.add(attribute='vale', attribute_n='value')`
**NOTE**: Attributes with `-`, like `max-limit` use underscore `_`: `max_limit`

`list_queues.add(name='001', max_limit='512k/4M', target='192.168.10.1/32')``list.set(id, attributes)``list_queues.set(id='*2', name='john')``list.get(attribute=value)``list_queues.get(name='john')``list.remove(id)``list_queues.remove(id='*2')``connection.disconnect()`
...
[{'id': '*B', 'name': 'ether10', 'poe-out': 'off', 'poe-priority': '10', 'poe-lldp-enabled': 'false', 'power-cycle-ping-enabled': 'false', 'power-cycle-interval': 'none'}]>> api.get_resource('/interface/ethernet/poe').call('set', {'poe-out': 'off', '.id': "*B"})
```
```
